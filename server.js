...
client.on("message", (topic, payload, packet) => {
  ...
  console.log(`Incomming payload on ${topic}:`, payload);

  const macId = topic.split("/")[1];

  const responseTopic = `gateway/${macId}/advertisements`;
  ...
  // Save message
  // saveMessage({ topic, macId, message, timestamp: new Date().toISOString(), retained: packet.retain });
});
